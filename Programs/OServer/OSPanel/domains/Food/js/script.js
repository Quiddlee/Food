...
function calculator() {
  ...
  const getDynamicInformation = selector => {
    ...
    input.addEventListener('input', () => {
      if (input.value.match(/\D/g)) input.style.border = '1px solid red';else input.style.border = 'none';
...
function cards() {
  ...
  axios.get('http://localhost:3000/menu').then(data => {
    // мы берём каждый элемент массива - объект, и деструктуризируем его на свойства
...
function forms() {
  ...
  const message = {
    ...
    // что бы использовать изображения в js нужно просто написать путь к нему
  ...
  const postData = async (url, data) => {
    // там где установлен оператор await, пока асинхронный код не выполнится
    const res = await fetch(url, {
      method: 'POST',
      headers: {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      }).catch(() => showThanksModal(message.failure)).finally(() => form.reset()); // Чтобы инпуты работали корректно, обязательно в верстке
...
function slider() {
  ...
  if (slides.length < 10) totalSlides.textContent = `0${slides.length}`;else totalSlides.textContent = slides.length;
  ...
  next.addEventListener('click', () => {
    if (offset === slidesWidth * slidesNumber) offset = 0;else offset += slidesWidth;
  ...
  prev.addEventListener('click', () => {
    if (offset === 0) offset = slidesWidth * slidesNumber;else offset -= slidesWidth;
    slidesField.style.transform = `translateX(-${offset}px)`;
...
function timer() {
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      // + используем уже полученые элементы со страницы (days, houre, etc)
